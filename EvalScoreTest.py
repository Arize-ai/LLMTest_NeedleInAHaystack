...
information is not available in the context respond UNANSWERABLE.'''
    ...
    def run_test(self):
        ...
        for context_length in self.context_lengths:
            for run_number in range(self.number_of_runs_per_context_length):
                ...
                for depth_percent in self.document_depth_percents:
                    ...
                    results = self.create_contexts(needle_rnd_number, insert_needle, random_city, trim_context, context_length, depth_percent, run_number)
        ...
        run_name = ("template_ver_" + self.template_version + "_" + self.model_provider + '_' + self.model_name + "_" + str(self.context_lengths_num_intervals)  + "_" + str(self.document_depth_percent_intervals) ).replace("/", "_")
        df = pd.concat([df, needle_test_results], axis=1)
        self.plot_point_distribution(df, "score", "error_corruption_percentage", run_name, jitter_magnitude=0.05,circle_size=500)
    ...
    def create_contexts(self, needle_rnd_number, insert_needle, random_city, trim_context, context_length, error_percent, run_number):
    ...
    def insert_errors_in_paragraph(self, paragraph, percent_error):
        ...
        for i in error_indices:
            words[i] = self.insert_error_in_word(words[i])
    ...
    def insert_error_in_word(self, word):
            ...
            if error_type == "swap":
                ...
                if len(word) > 1:
                    ...
                    return (word[:swap_index] + word[swap_index + 1] + word[swap_index] +
                            word[swap_index + 2:])
                else:
            ...
            if error_type == "double":
                ...
                return word[:double_index] + word[double_index] * 2 + word[double_index + 1:]
    ...
    def plot_point_distribution(self, dataframe, x_column, y_column, run_name, jitter_magnitude=0.15, circle_size=225):
        """
        ...
        for i, (idx, row) in enumerate(df_mean.iterrows()):
            ...
            jittered_x = df_category[x_column] + np.random.uniform(-jitter_magnitude, jitter_magnitude, size=len(df_category))
            ax.scatter(y=np.repeat(i, df_category.shape[0]), x=jittered_x, s=circle_size, edgecolors='gray', color=(1, 0.5, 0, 0.5), alpha=0.5)
            ax.scatter(y=i, x=df_median.loc[idx, x_column], s=circle_size, c='firebrick')

        # Annotate
        ax.text(x_max * 0.8, len(df_mean) / 2, "$red \; dots \; are \; the \: median$", fontdict={'size':12}, color='firebrick')

        # Set y-ticks to correspond to the group names (in reverse order)
        ax.set_yticks(range(len(df_mean)))
        ax.set_yticklabels([f"{idx:.1f}" for idx in df_mean.index], fontdict={'horizontalalignment': 'right'}, alpha=0.7)

        # Decorations
        red_patch = plt.plot([],[], marker="o", ms=10, ls="", mec=None, color='firebrick', label="Median")
        orange_patch = plt.plot([],[], marker="o", ms=10, ls="", mec=None, color=(1, 0.5, 0, 0.5), label="Data Points")
